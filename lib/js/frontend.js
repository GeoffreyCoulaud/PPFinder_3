...
let vueContainer, userOptions, languageMixin;
...
function onDocReady(){
	...
	function initLanguage(){
		// Make the lang property accessible by all elements
	...
	function initVue(){
		...
		// Handle search DB request
		vueContainer.$on('search', searchDB);
		
		// Handle language change
		vueContainer.$on('languageChange', function(lang){
			ipcRenderer.on('languageChangeReply', ()=>{
		...
		vueContainer.$on('scanBeatmaps', scanBeatmaps);
	}
...
class SearchRequest {
	constructor(id, page = 0, criteria = null){
		this.id       = id;
...
function getSearchCriteria(){
	...
	function getParam(name){
		const component = vueContainer.$children.find((x)=>{
			// A search input has the data 'role' === 'input'
			if (typeof x.role !== 'string'){return false;}
			return (x.role==='input' && x.name===name);
		});
...
function searchDB(){
	...
	const req      = new SearchRequest(ID, currentPage, criteria);
...
function handleSearchResults(results, page = 0, maxPage = 0, nPerPage = 50){
	// Log the results
	console.clear();
	...
	vueContainer.results = results;

	// Update page navigation
	// TODO
	console.log('Pagination', page, maxPage, nPerPage);
}

ipcRenderer.on('searchBeatmapsReply', function(event, {id, page, maxPage, nPerPage, results}){
	...
	for (let i=lastSearchIndex; i>=0; i--){
		...
		if (elem.id === id){ 
			...
			pos = i;
			hideSearchLoader();
			handleSearchResults(results, page, maxPage, nPerPage);
		} 
...
function scanBeatmaps(){
	...
	// Show the scan popup
	const menuLoadMaps = vueContainer.$children.find(x=>x.type==='menuLoadMaps');
