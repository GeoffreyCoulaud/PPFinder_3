...
function onDocReady(){
	...
	function initVue(){
		...
		buttonsVue.$children.find(x=>x.type==="buttonOptions").$children.find(x=>x.type==="optionsMenu").$children.find(x=>x.name==="languageSelect").select(userOptions.currentLanguage);
...
function scanBeatmaps(){
	...
	ipcRenderer.on('stateScanBeatmaps', function(event, data){
		// Update the popup with the given data
	});
}
